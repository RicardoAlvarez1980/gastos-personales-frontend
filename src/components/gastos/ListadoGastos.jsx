...
export default function ListadoGastos({ anio, mes, gastos, loading, error, onEliminar, onEditar }) {
  ...
  return <section className="v2-gastos-panel">
    ...
    {gastoEliminar && <Modal title="¿Eliminar este gasto?" icon="×" tone="danger" onCancel={() => !eliminando && setGastoEliminar(null)} onConfirm={confirmarEliminacion} confirmLabel={eliminando ? 'Eliminando…' : 'Eliminar'}><div className="v2-modal-context"><strong>{nombreServicio(gastoEliminar.servicios?.nombre || gastoEliminar.servicio_nombre, gastoEliminar.servicio_id)}</strong><span>{meses[Number(gastoEliminar.mes)] || ''} {gastoEliminar.año}</span></div><div className="v2-delete-amount">{importe(gastoEliminar.importe)}</div><p className="v2-modal-hint">Esta acción no se puede deshacer.</p>{errorAccion && <div className="v2-error">{errorAccion}</div>}</Modal>}
  </section>;
}
