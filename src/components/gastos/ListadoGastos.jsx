...
export default function ListadoGastos({ anio, mes, gastos, loading, error, onEliminar, onEditar, anual = false, titulo = 'GASTOS REGISTRADOS', total = 0 }) {
  ...
  const totalesPorMes = mesesConGastos.map(mesNumero => gastos.filter(g => Number(g.mes) === mesNumero).reduce((sum, g) => sum + Number(g.importe || 0), 0));
  const promedioMensual = anual && totalesPorMes.length ? totalesPorMes.reduce((sum, importe) => sum + importe, 0) / totalesPorMes.length : 0;
  ...
  return <section className="v2-gastos-panel">
    ...
    {!loading && !error && gastos.length > 0 && <div className={anual ? 'v2-summary-grid v2-summary-grid-annual' : 'v2-summary-grid'}>
      {anual && <div className="v2-summary-card"><span>Meses registrados</span><strong>{mesesRegistrados}</strong><small>Con gastos en {anio}</small></div>}
