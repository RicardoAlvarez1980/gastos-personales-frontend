...
export default function ListadoGastosV2({ anio, mes, gastos, loading, error, onEliminar, onEditar }) {
  ...
  return <section className="v2-gastos-panel">
    ...
    {gastoEditar && <ModalV2 title="Editar importe" onCancel={() => !editando && setGastoEditar(null)} onConfirm={confirmarEdicion} confirmLabel={editando ? 'Guardando…' : 'Guardar cambios'}><p><strong>{nombreServicio(gastoEditar.servicios?.nombre || gastoEditar.servicio_nombre, gastoEditar.servicio_id)}</strong> · {meses[Number(gastoEditar.mes)] || ''} {gastoEditar.año}</p><label className="v2-edit-label"><span>Nuevo importe</span><div className="v2-money-input"><b>$</b><input autoFocus type="number" min="0" step="0.01" value={nuevoImporte} onChange={(e) => setNuevoImporte(e.target.value)} /></div></label>{errorAccion && <div className="v2-error">{errorAccion}</div>}</ModalV2>}
    {gastoEliminar && <ModalV2 title="¿Eliminar este gasto?" onCancel={() => !eliminando && setGastoEliminar(null)} onConfirm={confirmarEliminacion} confirmLabel={eliminando ? 'Eliminando…' : 'Eliminar'}><p><strong>{nombreServicio(gastoEliminar.servicios?.nombre || gastoEliminar.servicio_nombre, gastoEliminar.servicio_id)}</strong> · {meses[Number(gastoEliminar.mes)] || ''} {gastoEliminar.año}</p><p>Importe: <strong>{importe(gastoEliminar.importe)}</strong></p><p>Esta acción no se puede deshacer.</p>{errorAccion && <div className="v2-error">{errorAccion}</div>}</ModalV2>}
  </section>;
}
