...
function Inicio({ periodos, gastosState, onEliminar }) {
  const { gastos } = gastosState; const total = gastos.reduce((sum, gasto) => sum + Number(gasto.importe || 0), 0); const mesNombre = periodos.mes ? mesesNombre[periodos.mes - 1] : '—';
  return <section className="v2-dashboard"><div className="v2-welcome"><div><span className="v2-eyebrow">RESUMEN</span><h2>Mis gastos</h2><p>Una vista clara de lo que realmente registraste.</p></div><SelectorPeriodos {...periodos} /></div>{periodos.error && <div className="v2-error">{periodos.error}</div>}<div className="v2-cards"><article><span>Total del período</span><strong>{formatearImporte(total)}</strong><small>{periodos.mes ? `${mesNombre} ${periodos.anio}` : 'Sin período seleccionado'}</small></article><article><span>Meses cargados</span><strong>{periodos.meses.length}</strong><small>{periodos.anio ? `Con datos en ${periodos.anio}` : 'Seleccioná un año'}</small></article><article><span>Gastos registrados</span><strong>{gastos.length}</strong><small>{periodos.mes ? mesesNombre[periodos.mes - 1] : 'Mes'} {periodos.anio || ''}</small></article></div><ListadoGastos anio={periodos.anio} mes={periodos.mes} gastos={gastos} loading={gastosState.loading} error={gastosState.error} onEliminar={onEliminar} onEditar={gastosState.editarImporte} /></section>;
}
...
export default function App() {
  const [section, setSection] = useState('inicio'); const [gastoGuardado, setGastoGuardado] = useState(null); const periodos = usePeriodos(); const gastosState = useGastos(periodos.anio, periodos.mes); const servicios = useServicios();
  const refrescarTodo = async (anioPreferido = periodos.anio, mesPreferido = periodos.mes) => { await periodos.refrescar({ anioPreferido, mesPreferido }); };
  const eliminar = async (id) => { const ok = await gastosState.eliminar(id); if (ok) await periodos.refrescar({ anioPreferido: periodos.anio, mesPreferido: periodos.mes }); return ok; };
  const handleGastoGuardado = async (nuevo) => { await refrescarTodo(nuevo.año, nuevo.mes); setSection('gastos'); setGastoGuardado(nuevo); };
  const mutation = useAgregarGasto({ onSuccess: handleGastoGuardado }); const servicioGuardado = gastoGuardado ? servicios.servicios.find(s => Number(s.id) === Number(gastoGuardado.servicio_id)) : null; const cerrarExito = () => setGastoGuardado(null); const irAlListado = async () => { setGastoGuardado(null); await gastosState.recargar(); setSection('gastos'); };
  return <div className="v2-shell"><aside className="v2-sidebar"><div className="v2-brand"><span className="v2-brand-mark">$</span><div><strong>Gastos</strong><small>PERSONALES · V2</small></div></div><nav>{menu.map(item => <button key={item.id} className={section === item.id ? 'active' : ''} onClick={() => setSection(item.id)}><span>{item.icon}</span>{item.label}</button>)}</nav><div className="v2-sidebar-footer">Versión 2.0 · base inicial</div></aside><main className="v2-main"><header className="v2-header"><div><span className="v2-eyebrow">CONTROL PERSONAL</span><h1>{menu.find(item => item.id === section)?.label}</h1></div></header>{section === 'inicio' ? <Inicio periodos={periodos} gastosState={gastosState} onEliminar={eliminar} /> : section === 'gastos' ? <section className="v2-dashboard"><div className="v2-page-toolbar"><div><span className="v2-eyebrow">HISTORIAL</span><h2>Gastos registrados</h2></div><SelectorPeriodos {...periodos} /></div><ListadoGastos {...gastosState} anio={periodos.anio} mes={periodos.mes} onEliminar={eliminar} onEditar={gastosState.editarImporte} /></section> : section === 'agregar' ? <Agregar periodos={periodos} servicios={servicios.servicios} mutation={mutation} /> : <Placeholder section={section} />}</main>{gastoGuardado && <Modal title="Gasto agregado correctamente" icon="✓" tone="edit" onCancel={cerrarExito} onConfirm={irAlListado} confirmLabel={`Ver gastos de ${mesesNombre[Number(gastoGuardado.mes) - 1]}`}><div className="v2-modal-context"><strong>{servicioGuardado ? formatearNombreServicio(servicioGuardado.nombre) : 'Gasto registrado'}</strong><span>{mesesNombre[Number(gastoGuardado.mes) - 1]} {gastoGuardado.año}</span></div><div className="v2-delete-amount">{formatearImporte(gastoGuardado.importe)}</div><p className="v2-modal-hint">El gasto fue guardado correctamente. Podés revisar todos los gastos de este período.</p></Modal>}</div>;
}
